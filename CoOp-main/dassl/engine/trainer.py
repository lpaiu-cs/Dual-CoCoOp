...
class SimpleTrainer(TrainerBase):
    ...
    def build_model(self):
        """사용할 모델, optimizer, scheduler 생성 및 등록
        ...
        if cfg.MODEL.INIT_WEIGHTS: # 필요시 사전학습 가중치 로드
            load_pretrained_weights(self.model, cfg.MODEL.INIT_WEIGHTS)
        self.model.to(self.device)
        print(f"# params: {count_num_param(self.model):,}")
        self.optim = build_optimizer(self.model, cfg.OPTIM) # optimizer 생성
        self.sched = build_lr_scheduler(self.optim, cfg.OPTIM) # scheduler 생성
        self.register_model("model", self.model, self.optim, self.sched) # 모델 등록

        device_count = torch.cuda.device_count() # GPU 할당
        if device_count > 1:
    ...
    def before_train(self):
        """
        1. 학습 재시작 여부 판단
        2. 텐서보드용 writer 초기화(그래프, loss, accuracy 등 기록용)
        3. 학습 시작 시간 기록
        """
    ...
    def after_epoch(self):
        """주기/마지막 에폭마다 checkpoint 저장 및 best model 저장"""
